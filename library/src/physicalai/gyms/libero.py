...
Note:
    ...
    - Images fed to policies are in raw robosuite orientation (no rotation), matching
      the LeRobot training convention for HuggingFaceVLA/smolvla_libero.
      The 180° flip is applied only in render() for human-viewable output.
...
class LiberoGym(Gym):
    ...
    def reset(
        ...
        **reset_kwargs: Any,  # noqa: ANN401, ARG002
        ...
        # Reset first, then apply the task-specific init state (matches LeRobot order).
        # Calling reset() after set_init_state() would discard the init state.
    ...
    def _format_raw_obs(self, raw_obs: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        # Pass raw images to the policy without rotation (matches LeRobot training convention).
        images = {}
    ...
    def render(self) -> np.ndarray:
        ...
        # Rotate 180° for human-viewable output only (matches LeRobot's render()).
