...
class SmolVLAModel(Model):
    ...
    def forward(
        ...
        Args:
            batch: Dictionary containing input tensors with keys:
                ...
                - EXTRA + ".action_is_pad": Optional padding mask for actions
    ...
    def compute_loss(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, torch.Tensor | float]]:
        ...
        if actions_is_pad is None:
            ...
            # Divide by the number of *valid* elements, not the full tensor.
            # A plain .mean() would count the zeroed-out padded steps in the
            # denominator and shrink the loss (and gradient) in proportion to
...
class VLAFlowMatching(SnapFlowModelMixin, nn.Module):
    ...
    def forward(
        ...
        return self.snapflow_mixed_loss(
            u_t=u_t,
            x_t=x_t,
            time=time,
            actions=actions,
